...
Review.create({user_id:2, anime_id:138 ,user_rating:9, user_start_date:, user_end_date:, body:"Brotherhood is just as entertaining and involving as its predecessor, and it's a testament to Arakawa's skill as a mangaka that she has been able to produce a tale that, at the very least, rivals the original anime adaptation.Yes, Brotherhood is more typically shounen than the other version, but the nice thing about this is that fans are given two very good versions of the same story, and that is something rare in anime."})
Review.create({user_id:3, anime_id:138 ,user_rating:8, user_start_date:, user_end_date:, body:"Fullmetal Alchemist: Brotherhood gets an immense amount of praise in the MAL community, is the #1 ranked show and is constantly referred to as a masterpiece and the greatest show ever created. I've seen many fans preach about how it lives up to the hype and can never receive too much praise. Now this is just the opinion of one guy. I'm certainly not the law of the land or anything. However, I personally feel as though calling FMA:B a masterpiece and the champion of all shows is a bit of a stretch."})
Review.create({user_id:4, anime_id:35 ,user_rating:10, user_start_date:, user_end_date:, body:"I laughed...
I cried...
...
Review.create({user_id:2, anime_id:30 ,user_rating:10, user_start_date:, user_end_date:, body:"I nearly skipped this gem just because of the music tag. Not really big into music anime but somehow the art dragged me into watching the first few episodes. First episode was an average episode with few interesting plot lines. But the second episode was the hook. Irony. The tag that made me nearly skip this anime was the one that kept me in it. The music, the music... THE MUSIC.. what can I say? It was just right down my taste. "})
Review.create({user_id:3, anime_id:30 ,user_rating:9, user_start_date:, user_end_date:, body:"Even though the story isn't anything new, the way it was written and directed make it feel like one. It is a story that revolves around music and how it connects people and how it affects them. It does not only affect them in a good way, but in a bad way too. And with that premise, a beautiful story about tennagers started."})
Review.create({user_id:4, anime_id:30 ,user_rating:7, user_start_date:, user_end_date:, body:"here are mainly only two sides to the debate on this show -- those that absolutely love it, and those that strongly dislike it. Is there an abundance of melodrama? Yes. Does this show try too hard at some points? Sure it does. Does that mean that this show is bad? Not necessarily. This show is neither the best thing ever, as some people seem to be saying, but it's not the worst either. Basically, what I'm about to do is help find some middle ground between the two viewpoints."})
Review.create({user_id:5, anime_id:123 ,user_rating:9, user_start_date:, user_end_date:, body:"Ookami Kodomo no Ame to Yuki translates to Wolf Children Ame and Yuki. What the film's title promises is accurate, but this is secondary to what the film is actually about. This is a movie entirely about the enduring and triumphant nature of maternal love.
...
Review.create({user_id:3, anime_id:66 ,user_rating:8, user_start_date:, user_end_date:, body:"I started off watching this anime because my brother recommended me it, so if it wasn't for him, I wouldn't have watched it. I started off watching the first season, and I just thought that the art was pretty good, it wasn't the best but the main characters attracted me, Chihaya mostly. So therefore, I really people out there to watch this anime too, or give it a try because it's not too shabby."})
Review.create({user_id:4, anime_id:66 ,user_rating:9, user_start_date:, user_end_date:, body:"Twenty five more episodes worth of charming romance and competitive karuta fun? Yes please. But don't expect a balance between the two; there's more latter quantity than the former, and whether you should be satisfied or not is up to your preference!
"})
Review.create({user_id:5, anime_id:66 ,user_rating:7, user_start_date:, user_end_date:, body:"If there is one word that describes Chihayafuru, it is passion - indeed, Chihaya furu means passionate. On the surface there is little that separates it from typical sports anime, from the focus on the team and team spirit to rivalries to tournament story arcs where you're typically served the main characters' opponents' motivations and reasons for participating in this sport."})
Review.create({user_id:7, anime_id:104 ,user_rating:8, user_start_date:, user_end_date:, body:"You have just stumbled upon the ultimate Mahou Shoujo, one that doesn't care if it's morally ambiguous or not, one with no stock footage, and one where the magic used substitutes candy for bloodbaths.
"})
Review.create({user_id:8, anime_id:104 ,user_rating:9, user_start_date:, user_end_date:, body:"It's been a long while since I watched and completed a magical girl series. So when I first read about SHAFT's first original anime production entitled Puella Magi Madoka Magica I was a bit skeptical at first, seeing how I usually don't watched the genre but at the same time curious on how SHAFT and Shinbou Akiyuki will gonna make one with their very unique visual style that defines SHAFT animated works. After watching their previous magical and supernatural series like Bakemonogatari, Dance in The Vampire Bund, Negima, and Natsu no Arashi, I'm convinced that I should at least try the series.
"})
Review.create({user_id:9, anime_id:107 ,user_rating:10, user_start_date:, user_end_date:, body:"I don't think I've ever given perfect 10's across a rating scale. I don't think the third Madoka Magica movie deserves 10's across the board either, but this is the closest I'll probably ever get.
...
Review.create({user_id:10, anime_id:107 ,user_rating:8, user_start_date:, user_end_date:, body:"The final chapter in the highly acclaimed Madoka trilogy/show has come to a close, and studio Shaft has closed this book right (if not heart wrenching). The story is all tied to Homura after the events of the first two films. We follow her as the story travels down a road most fans never saw coming, but since this is the final chapter there is an end to this road."})
Review.create({user_id:11, anime_id:106 ,user_rating:8, user_start_date:, user_end_date:, body:"Recaps are usually episodes/movies that re-tell a story that has already been told, but in a shorter and more compact way. It tells you valid points and informative parts in a story without all the unnecessary bits.
...
Review.create({user_id:1, anime_id:106 ,user_rating:8, user_start_date:, user_end_date:, body:"This movie is essentially another recap of the TV series and thus, there is no original material in terms of storytelling or plot twists."})
Review.create({user_id:2, anime_id:28 ,user_rating:, user_start_date:, user_end_date:, body:"One phrase that would perfectly describe Shirobako is simply ingenious. Surprisingly, the series has cleverly put together a lot of elements into one stand-out show. Aside from being an exposition of how anime series are made, it also tells us a cute and charming story all while boasting a splendid cast of characters and vibrant, dynamic designs.
...
Review.create({user_id:3, anime_id:28 ,user_rating:10, user_start_date:, user_end_date:, body:"Shirobako is a love letter to the anime industry. It doesn't glorify it or portray animators as hyper-talented geniuses, and many of the characters in the show admit they do not even understand why they're working in the industry. But there's something that keeps them passionate about what they do, even if it may not be the most respected form of media out there. "})
Review.create({user_id:4, anime_id:28 ,user_rating:9, user_start_date:, user_end_date:, body:"An anime about the creation process of an anime - or some kind of meta-animation or animeception - that is not only a daring approach to new age infotainment, but also an excellent opportunity for the creators to put in their passion associated with this kind of work, and to tell a realistic tale on the hardships that newbies in this industry are likely to encounter."})
Review.create({user_id:5, anime_id:77 ,user_rating:9, user_start_date:, user_end_date:, body:"he most common criticism I've seen so far of Kuroko no Basket has been how unrealistic it is. I myself am an avid basketball fan who plays regularly and watches NBA games. I can understand the criticism as this anime has a lot of spectacular dunks, blocks and flashy passes, the stuff you usually only see in highlight reels, but occurs regularly throughout the games in here. There's also the shounen elements with the special powers of Generation of Miracles members and other things such as the "martial arts" Seiho employs.
That being said, I am completely ok with this."})
Review.create({user_id:6, anime_id:77 ,user_rating:9, user_start_date:, user_end_date:, body:"If your a sports anime fan then 2 words automatically...no even magically came to your mind, Slam Dunk. However sports anime fans or not im sure realize just how awesome this anime is. This could very well be Slam Dunk Successor."})
Review.create({user_id:7, anime_id:113 ,user_rating:9, user_start_date:, user_end_date:, body:"As foreshadowed by the prequel OVA, Spice and Wolf II focus heavily on the romance aspect between our protagonists. By and large, this second adaptation has been regarded as equally successful as its predecessor (if not more successful). In terms of the light novels adaptation, it has been widely accepted for its accurate following of the “actual” story, unlike most anime adaptation. While previous knowledge of the series is not absolutely required to enjoy the show, it is highly recommended for viewers to watch the first season"})
Review.create({user_id:8, anime_id:113 ,user_rating:8, user_start_date:, user_end_date:, body:"Continuing a beloved series and making it greater or just as great can be a challenging task to take. This is not much of a problem for anime that are step-by-step adaptations of any type of original source medium. Spice and Wolf, if you don’t know, is one series that won me over from the previous season. Likewise, I was hoping that this 2nd season would have the same positive impact that the 1st season did for me. "})
Review.create({user_id:10, anime_id:93 ,user_rating:9, user_start_date:, user_end_date:, body:"I don't usually watch the romantic comedy genre of things. I always enjoyed action, fantasy, adventure like Soul Eater, Cowboy Bebop, Trigun etc. but for some reason I was in the mood for something different.

Toradora is just the 3rd romcom anime series i watched and I can truly say that it has become my FAVORITE anime."})
Review.create({user_id:1, anime_id:93 ,user_rating:9, user_start_date:, user_end_date:, body:"Toradora!= Another normal romance/drama anime involving high school...We had many anime of this kind. In fact, these type of anime are very common nowadays. I had watched this type many times, but mostly ended up so wrongly.
...
Review.create({user_id:2, anime_id:24 ,user_rating:8, user_start_date:, user_end_date:, body:"At the end of Unlimited Blade Works we were left with a rather monotonous and filler-full episode.
